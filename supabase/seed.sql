...
) as v(tag, title, "desc", features, wamsg, bg_color, badge_color, icon_name, "order", active)
where not exists (select 1 from public.nexor_biosite_services);

insert into public.nexor_biosite_faq_items (question, answer, "order", active)
select * from (values
...
) as v(question, answer, "order", active)
where not exists (select 1 from public.nexor_biosite_faq_items);

insert into public.nexor_biosite_plan_items (name, featured, features, "order", active)
select * from (values
...
) as v(name, featured, features, "order", active)
where not exists (select 1 from public.nexor_biosite_plan_items);

insert into public.nexor_biosite_site_content (section, key, value, value_type)
values
('general', 'whatsapp_number', '553198261608', 'text'),
('general', 'instagram_handle', 'nexordigitalgroup', 'text'),
('general', 'logo_url', '/assets/logo-nexor.svg', 'image_url'),
('hero', 'badge_text', 'NEXOR Digital Group', 'text'),
('cta', 'title', 'Pronto para transformar sua bio em vendas?', 'text'),
('cta', 'subtitle', 'Fale com a NEXOR e tenha um biosite profissional, bonito e simples de atualizar.', 'text'),
('cta', 'button_text', 'Quero meu biosite', 'text'),
('cta', 'wamsg', 'Ola%2C%20quero%20criar%20meu%20biosite%20com%20a%20NEXOR.', 'text'),
('footer', 'tagline', 'Solucoes digitais para pequenos negocios venderem melhor.', 'text'),
('footer', 'copyright', 'NEXOR Digital Group. Todos os direitos reservados.', 'text')
